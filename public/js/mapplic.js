...
; (function ($) {
	...
	var Mapplic = function (element) {
		...
		function Tooltip() {
			...
			this.show = function (location) {
				...
				if (location) {
					...
					} else if(location.category === "vendido"){
						this.link.attr('disabled', true).css('background-color', '').show();
						if (location.color) this.link.css('background-color', location.color);
					} else this.link.hide();
		...
		function Lightbox() {
			...
			var form = `<h3 class="text-center">Cotizar</h3> <form method="post" role="form">
			<div class="card-body">
				<div class="row">
					<div class="col-md-6">
				...
				<div class="form-group label-floating is-empty">
					<label for="exampleMessage1" class="bmd-label-floating">Teléfono</label>
			...
			<div class="card-footer justify-content-center">
				...
				<input type="submit" class="btn btn-primary pull-right" value="Contactar">
			</div>
		</form>`;
			
			var cal = `
			<label>Plan de Pagos</label>
			<select class="calculadora" data-style="select-with-transition" title="Single Select" data-size="7">
				<option value="0">Contado</option>
				<option value="1">12 meses</option>
				<option value="2">18 meses</option>
				<option value="3">24 meses</option>
				<option value="4">26 meses</option>
			</select>`;
			...
			this.init = function () {
				...
				this.row = $('<div></div>').addClass('row').appendTo(this.el);
				this.desc = $('<div></div>').addClass('col-12 col-md-6').appendTo(this.row);
				this.coti = $('<div></div>').addClass('col-12 col-md-6').appendTo(this.row);
				this.precio = $('<div></div>').addClass('precio').appendTo(this.desc);
